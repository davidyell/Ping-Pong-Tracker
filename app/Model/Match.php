 ...
 * Set the score
 *
 * @param array $result The result of the match
 * @return array
 */
 ...
 * @return void
 */
	protected function unsetBlankPlayers() {
 ...
 * @return void
 */
	protected function assignScores() {
 ...
 */
	protected function matchBreakdown() {
 ...
 */
	public function getGlobalStats() {
		...
		$stats = $this->MatchesPlayer->find('all', [
			'contain' => false,
			'fields' => $this->MatchesPlayer->Player->statsFields
		]);

		// Matches played by day
		$matchesByDay = $this->find('all', [
			'contain' => false,
			'conditions' => [],
			'fields' => [
		...
		$matchId = $this->MatchesPlayer->find('first', [
			'contain' => false,
			'fields' => ['match_id'],
			'order' => 'score DESC, created DESC'
		]);

		$highScoreMatch = $this->find('first', [
			'contain' => [
				'MatchesPlayer' => [
					'Player'
				],
			...
			'conditions' => [
				'Match.id' => $matchId['MatchesPlayer']['match_id']
			]
		]);
		...
		$return['highest_match_score'] = $highScoreMatch;
		$return['stats'] = $stats[0][0];
		$return['matches_by_day'] = $matchesByDay;
		$return['most_played_days'] = $mostMatches;
